...
genServerCrt(){
  # 生成根秘钥及证书
  openssl req -x509 -sha256 -newkey rsa:${rsa} -keyout ${dir}/ca.key -out ${dir}/ca.crt -days 3560 -nodes -subj '/CN=Launcher LStack Authority'
...
# 生成客户端密钥，证书并使用CA证书签名
genClientCert(){
  openssl req -new -newkey rsa:${rsa} -keyout ${dir}/client.key -out ${dir}/client.csr -nodes -subj "/CN=${CN}"
  openssl x509 -req -sha256 -days 3650 -in ${dir}/client.csr -CA ${dir}/ca.crt -CAkey ${dir}/ca.key -set_serial 02 -out ${dir}/client.crt
}
